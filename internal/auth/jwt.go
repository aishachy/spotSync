...
type JWTService interface {
	GenerateToken(userId uint, email, name string) (string, error)
...
func (js *jwtService) GenerateToken(userId uint, email, name string) (string, error) {
	claims := JWTClaims{
		UserId: userId,
		Name:   name,
		Email:  email,
...
func (js *jwtService) ValidateToken(tokenStr string) (*JWTClaims, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Unexpected signing ethod: %w", err)
	}
